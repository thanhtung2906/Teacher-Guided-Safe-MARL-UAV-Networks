import torch
import numpy as np
import time

# Import môi trường và class Actor từ code của bạn
from uav_env import UAVEmergencyEnv
from train_mappo import Actor

def main():
    DEVICE = "cuda" if torch.cuda.is_available() else "cpu"

    # 1. Khởi tạo môi trường ở chế độ Human (để xem đồ họa)
    env = UAVEmergencyEnv(render_mode="human")
    obs, infos = env.reset()
    
    # 2. Khởi tạo duy nhất mạng Actor (Không cần Critic)
    obs_dim = env.observation_space("uav_0").shape[0]
    actor = Actor(obs_dim).to(DEVICE)
    
    # 3. Load trọng số (Weights) từ file .pt
    print("Đang tải trọng số mô hình từ mappo_checkpoint.pt...")
    checkpoint = torch.load("mappo_checkpoint.pt", map_location=DEVICE)
    actor.load_state_dict(checkpoint["actor"])
    actor.eval() # Chuyển mạng sang chế độ đánh giá (Tắt dropout/batchnorm nếu có)

    # 4. Vòng lặp chạy thực tế
    total_reward = 0.0
    print("Bắt đầu Inference...")
    
    for step in range(env.time_slot):
        # Nếu tất cả UAV đã chết / hết pin thì dừng
        if not env.agents:
            break
            
        actions_dict = {}
        
        # --- Decentralized Execution ---
        # Từng UAV tự dùng Actor của mình để ra quyết định
        with torch.no_grad():
            for agent in env.agents:
                obs_array = obs[agent]
                obs_tensor = torch.as_tensor(obs_array, dtype=torch.float32, device=DEVICE)
                
                # Truyền qua mạng Actor
                d_move, d_srv, d_pow = actor.forward(obs_tensor)
                
                # TRICK QUAN TRỌNG: Lấy hành động tốt nhất (argmax) thay vì ngẫu nhiên (sample)
                a_move = torch.argmax(d_move.logits, dim=-1).item()
                a_srv = torch.argmax(d_srv.logits, dim=-1).item()
                a_pow = torch.argmax(d_pow.logits, dim=-1).item()
                
                actions_dict[agent] = np.array([a_move, a_srv, a_pow])
        
        # 5. Gửi hành động vào môi trường
        next_obs, rewards, terms, truncs, infos = env.step(actions_dict)
        
        # Render đồ họa (có thể thêm sleep để xem cho rõ)
        env.render()
        time.sleep(0.1) 
        
        # Cộng dồn phần thưởng (Lấy đại diện 1 UAV vì ta đang dùng Global Reward)
        if env.agents:
            total_reward += rewards[env.agents[0]]
            
        obs = next_obs
        
    print(f"\n[HOÀN THÀNH] Tổng phần thưởng (Reward) đạt được: {total_reward:.2f}")
    env.close()

if __name__ == "__main__":
    main()